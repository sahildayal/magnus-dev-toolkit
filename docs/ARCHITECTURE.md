...
2. **MCPs:** Auto-configures 10 core MCPs (GitHub, PostgreSQL, Playwright, Figma, Sentry, Chrome DevTools, Filesystem, Memory, Git, Docker), plus `mcp-router` itself as an 11th server. Fetch MCP is implemented but disabled by default (broken upstream dependency in `mcp-server-fetch` on PyPI).
