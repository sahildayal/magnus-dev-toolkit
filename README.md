...
| **Phase 2's Node.js auto-bootstrap for npm-installed core tools** (Claude Code) | `actions/setup-node` already puts npm on the CI runner before Phase 2 ever runs, so the bootstrap branch that installs Node.js on-demand when npm is missing never actually executes in CI - only the trivial "npm already present" path does | Untested on the one machine shape it exists for: a real fresh Windows install with no Node.js and no language selected. Logic is straightforward (reuses the same verified winget install path used everywhere else) but has no end-to-end proof |
